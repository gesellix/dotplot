 ...
 * 
 */
public final class FilterServiceTest extends TestCase {
	...
	public void testCreateTask() {
		try {
			...
			TokenStreamContext context = new TokenStreamContext(
					new ITokenStream() {
	...
	/*
	 * Test method for
	 * 'org.dotplot.tokenizer.FilterService.FilterService(String)'
	 */
	...
	public void testGetResultContext() {
		...
		try {
			...
			config = (IFilterConfiguration) this.context
			...
			config.setFilterParameter(FilterService.GENERAL_TOKEN_FILTER_ID,
					params);

			assertEquals(NullContext.context, this.service.getResultContext());
			this.service.run();
			assertFalse(NullContext.context.equals(this.service
					.getResultContext()));
			TokenStreamContext resultContext = (TokenStreamContext) this.service
					.getResultContext();
	...
	/*
	 * Test method for
	 * 'org.dotplot.tokenizer.FilterService.getResultContextClass()'
	 */
	public void testGetResultContextClass() {
		assertSame(TokenStreamContext.class, this.service
				.getResultContextClass());
	}

	/*
	 * Test method for
	 * 'org.dotplot.tokenizer.FilterService.registerDefaultConfiguration(IConfigurationRegistry)'
	 */
	public void testRegisterDefaultConfiguration() {
		this.service.registerDefaultConfiguration(this.context
				.getConfigurationRegistry());
		assertTrue(this.context.getConfigurationRegistry().getAll()
				.containsKey(FilterService.FILTER_CONFIGURATION_ID));
		assertNotNull(this.context.getConfigurationRegistry().getAll().get(
				FilterService.FILTER_CONFIGURATION_ID));
	}

	/*
	 * Test method for
	 * 'org.dotplot.tokenizer.FilterService.workingContextIsCompatible(Class)'
	 */
