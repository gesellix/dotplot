 ...
 *
 */
public class PluginLoadingTaskPartTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		this.part = new PluginLoadingTaskPart("pluginloader", this.dir, this.context);
		this.context.getServiceRegistry().register("org.dotplot.core.pluginloader",new TestService("org.dotplot.core.pluginloader"));
		this.context.getJobRegistry().register("org.dotplot.test.testjob4", new BatchJob());
	}

	/*
	 * Test method for 'org.dotplot.core.PluginLoadingTaskPart.PluginLoadingTaskPart(String, DirectoryRessource, DotplotContext)'
	 */
	public void testPluginLoadingTaskPart() {
		...
		try {
			PluginLoadingTaskPart part = new PluginLoadingTaskPart("pluginloader", this.dir, this.context);
	...
	/*
	 * Test method for 'org.dotplot.core.PluginLoadingTaskPart.setLocalRessources(Collection<IRessource>)'
	 */
	...
	public void testRun() {
		try {
			...
			this.context.getPluginRegistry().register("org.dotplot.testplugin", new IPlugin(){
				...
				public void storeExtention(String serviceID, String hotSpotID, Extention extention) {}
			...
			Plugin plugin = (Plugin) ((Map)this.part.getResult()).get("org.dotplot.core.test");
			...
			assertEquals("Einführender Test für die plugin.xml",plugin.getInfo());
			
			assertEquals(1, plugin.getServiceRegistry().getAll().size());
			IService service = plugin.getServiceRegistry().get("org.dotplot.core.pluginloader2");
			...
			assertTrue(plugin.getExtentions().containsKey("org.dotplot.core.pluginloader"));
			assertEquals(2, plugin.getExtentions().get("org.dotplot.core.pluginloader").size());
			assertTrue(plugin.getExtentions().get("org.dotplot.core.pluginloader").containsKey("object1"));
			assertTrue(plugin.getExtentions().get("org.dotplot.core.pluginloader").containsKey("object2"));
			assertEquals(2, plugin.getExtentions().get("org.dotplot.core.pluginloader").get("object1").size());
			assertEquals(1, plugin.getExtentions().get("org.dotplot.core.pluginloader").get("object2").size());
			
			Iterator<Extention> iter;
			iter = plugin.getExtentions().get("org.dotplot.core.pluginloader").get("object1").iterator();
			assertTrue(iter.next().getExtentionObject() instanceof String);
			assertTrue(iter.next().getExtentionObject() instanceof String);
			assertFalse(iter.hasNext());
			
			iter = plugin.getExtentions().get("org.dotplot.core.pluginloader").get("object2").iterator();
			assertTrue(iter.next().getExtentionObject() instanceof String);
			assertFalse(iter.hasNext());

			assertNotNull(plugin.getJobRegistry().getAll());
			assertEquals(1, plugin.getJobRegistry().getAll().size());
			assertTrue(plugin.getJobRegistry().getAll().containsKey("org.dotplot.test.testbatchjob"));
			
			assertTrue(plugin.getJobRegistry().getAll().get("org.dotplot.test.testbatchjob") instanceof BatchJob);
			
			BatchJob job;
			job = (BatchJob)plugin.getJobRegistry().getAll().get("org.dotplot.test.testbatchjob");
			assertEquals(1, job.getServiceBatch().size());
			assertEquals("org.dotplot.core.pluginloader2", job.getServiceBatch().get(0));
		}				
		catch (Exception e) { 
			fail("no exception:" + e.getClass().getName() + ":"
					+ e.getMessage());
