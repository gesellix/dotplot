 ...
 * 
 */
public final class PluginTest extends TestCase {
	...
	/*
	 * Test method for 'org.dotplot.core.DotplotPlugin.DotplotPlugin(String,
	 * String)'
	 */
	...
	public void testGetStoreExtention() {
		...
		try {
			...
			assertTrue(this.plugin.getExtentions().get("test").containsKey(
					"test"));
			assertEquals(1, this.plugin.getExtentions().get("test").get("test")
					.size());
			...
			assertTrue(this.plugin.getExtentions().get("test").containsKey(
					"test"));
			assertEquals(2, this.plugin.getExtentions().get("test").get("test")
					.size());
			...
			assertEquals(2, this.plugin.getExtentions().get("test").get("test")
					.size());
			assertEquals(1, this.plugin.getExtentions().get("test")
					.get("test2").size());
			...
			assertEquals(1, this.plugin.getExtentions().get("test")
					.get("test2").size());
			assertTrue(this.plugin.getExtentions().get("test2").containsKey(
					"test2"));
			assertEquals(1, this.plugin.getExtentions().get("test2").get(
					"test2").size());
		}
		catch (Exception e) {
			fail("no exception:" + e.getClass().getName() + ":"
