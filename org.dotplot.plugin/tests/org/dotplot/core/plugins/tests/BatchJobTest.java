 ...
 * 
 */
public final class BatchJobTest extends TestCase {
	...
	private class TestService extends AbstractService implements
		...
		public IContext getResultContext() {
			...
			if (this.getWorkingContext() instanceof ResultContext) {
				buffer.append(((ResultContext) this.getWorkingContext())
						.getResult());
			}
		 ...
		 * 
		 * @see
		 * org.dotplot.services.ITaskResultMarshaler#marshalResult(java.util
		 * .Map)
		 */
	...
	/*
	 * Test method for 'org.dotplot.services.BatchJob.process(ServiceManager,
	 * FrameworkContext)'
	 */
	public void testProcess() {
		...
		try {
			this.frameworkContext.getServiceRegistry().register(
					"test1",
					new TestService("test1", new ResultContext1(),
							new NullContext()));
			this.frameworkContext.getServiceRegistry().register(
					"test2",
					new TestService("test2", new ResultContext2(),
							new ResultContext1()));
			this.frameworkContext.getServiceRegistry().register("test3",
					new TestService("test3", context, new ResultContext2()));
		}
		...
		assertTrue(this.job.validatePreconditions(this.frameworkContext
				.getServiceRegistry()));
		...
		assertEquals("", ((DefaultErrorHandler) this.job.getErrorHandler())
				.getErrorMessages());

		this.job.process(this.frameworkContext);
		assertEquals("[1](fatal) at Job:no id -> blub\n",
				((DefaultErrorHandler) this.job.getErrorHandler())
						.getErrorMessages());
		...
		try {
			this.frameworkContext.getServiceRegistry().register(
					"test4",
					new TestService("test4", new ResultContext1(),
							new ResultContext1()));
		}
		catch (DuplicateRegistrationException e) {
			fail("no exception");
		...
		buffer
				.append("[1](fatal) at Job:no id -> blub\n[2](fatal) at Job:no id -> ");
		buffer
				.append("org.dotplot.core.plugins.tests.BatchJobTest$ResultContext2\n");

		assertEquals(buffer.toString(), ((DefaultErrorHandler) this.job
	...
	/*
	 * Test method for
	 * 'org.dotplot.services.BatchJob.validatePreconditions(ServiceManager,
	 * FrameworkContext)'
	 */
	public void testValidatePreconditions() {
		...
		try {
			...
			this.frameworkContext.getServiceRegistry().register(
					"test3",
					new TestService("test3", new ResultContext1(),
							new ResultContext2()));
		}
		catch (DuplicateRegistrationException e) {
			fail("no exception");
		...
		this.job.addService("test3");
		assertFalse(this.job.validatePreconditions(this.frameworkContext
				.getServiceRegistry()));
