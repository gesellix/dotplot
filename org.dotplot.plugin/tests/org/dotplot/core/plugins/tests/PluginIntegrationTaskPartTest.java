 ...
 *
 */
public class PluginIntegrationTaskPartTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		this.context.getServiceRegistry().register("testservice1",new TestService("testservice1"));
		this.plugin = new Plugin("testplugin", "1.0");
		
		this.plugin.getJobRegistry().register("testjob", new BatchJob());
		this.plugin.getServiceRegistry().register("testservice2", new TestService("testservice2"));
		this.plugin.getServiceRegistry().register("testservice3",new TestService("testservice3"));
		this.plugin.storeExtention("testservice1", "spot1", new Extention(this.plugin, new Object()));
		this.plugin.storeExtention("testservice2", "spot1", new Extention(this.plugin, new Object()));
		
		Collection<IPlugin> plugins = new Vector<IPlugin>();
		plugins.add(this.plugin);
		
		this.part = new PluginIntegrationTaskPart("testpart", plugins, this.context);
	}

	/*
	 * Test method for 'org.dotplot.core.plugins.PluginIntegrationTaskPart.PluginIntegrationTaskPart(String, IPlugin)'
	 */
	...
	/*
	 * Test method for 'org.dotplot.core.plugins.PluginIntegrationTaskPart.setLocalRessources(Collection<IRessource>)'
	 */
	...
	/*
	 * Test method for 'org.dotplot.core.plugins.PluginIntegrationTaskPart.run()'
	 */
	public void testRun() {
		IService service; 
