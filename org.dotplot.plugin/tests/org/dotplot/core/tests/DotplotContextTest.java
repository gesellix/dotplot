...
public class DotplotContextTest extends TestCase {
	...
	private class TestJob implements IJob {
		...
		/* (non-Javadoc)
		 * @see org.dotplot.services.IJob#setErrorHandler(org.dotplot.services.IErrorHandler)
		 */
		...
		/* (non-Javadoc)
		 * @see org.dotplot.services.IJob#process(org.dotplot.services.ServiceManager, org.dotplot.services.IFrameworkContext)
		 */
		public boolean process(IFrameworkContext context) {
			this.buffer.append("process");
			this.buffer.append("\n");
			this.buffer.append(context.getServiceRegistry().getClass().getName());
			this.buffer.append("\n");
		...
		/* (non-Javadoc)
		 * @see org.dotplot.services.IJob#validatePreconditions(org.dotplot.services.ServiceManager, org.dotplot.services.IFrameworkContext)
		 */
	...
	protected void setUp() throws Exception {
		...
		this.context = new DotplotContext(this.workingDir.getAbsolutePath(), this.pluginDir.getAbsolutePath());
	}

	/*
	 * Test method for 'org.dotplot.core.DotplotContext.DotplotContext(IServiceManager, String)'
	 */
	public void testDotplotContext() {		
		...
		try {
			...
			assertEquals(workingDir.getAbsolutePath(), context.getWorkingDirectory());
		}
		catch (Exception e) {
	...
	public void testGetWorkingDirectory() {
		assertEquals(workingDir.getAbsolutePath(), this.context.getWorkingDirectory());
	}
	
	public void testGetTypeBindingRegistry(){
		try {
			ITypeBindingRegistry registry = this.context.getTypeBindingRegistry();
	...
	public void testCreateDotplotFile(){
		...
		try {
			...
			ITypeBindingRegistry registry2 = this.context.getTypeBindingRegistry();
