...
public class DirectoryJarClassLoaderTest extends TestCase {
	...
	/*
	 * Test method for 'testpackage.DirectoryJarClassLoader.DirectoryJarClassLoader(File)'
	 */
	public final void testDirectoryJarClassLoaderFile() {
		...
		try{
			...
			assertEquals("The assigned argument must be an existing directory", e.getMessage());
		}
		catch(Exception e){
			fail("IllegalArgumentException must be thrown");			
		}
	...
	/*
	 * Test method for 'testpackage.DirectoryJarClassLoader.DirectoryJarClassLoader(File, ClassLoader)'
	 */
	public final void testDirectoryJarClassLoaderFileClassLoader() {
		File directory = new File("testfiles");
		
		try{
			new DirectoryJarClassLoader(null, ClassLoader.getSystemClassLoader());
			fail("NullPointerException must be thrown");
		}
		catch(NullPointerException e){
			/*all clear*/
		...
		try{
			new DirectoryJarClassLoader(new File("testfiles/testplugin.xml"),ClassLoader.getSystemClassLoader());
			fail("IllegalArgumentException must be thrown");
		}
		catch(IllegalArgumentException e){
			assertEquals("The assigned argument must be an existing directory", e.getMessage());
		...
		try{
			...
			new DirectoryJarClassLoader(noDir, ClassLoader.getSystemClassLoader());
			fail("IllegalArgumentException must be thrown");
		}
		catch(IllegalArgumentException e){
			assertEquals("The assigned argument must be an existing directory", e.getMessage());
		...
		try{
			new DirectoryJarClassLoader(directory,ClassLoader.getSystemClassLoader());
			/*all clear*/
		}
		catch(Exception e){
			fail("No Exception must be thrown");			
	...
	public final void testFindClassString() {
		...
		try{
			loader = new DirectoryJarClassLoader(directory, ClassLoader.getSystemClassLoader());
