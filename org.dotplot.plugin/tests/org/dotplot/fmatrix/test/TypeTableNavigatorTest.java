 ...
 * @version 0.1
 */
public final class TypeTableNavigatorTest extends TestCase {
	...
	public void testGetPostings() {
		...
		assertEquals("expecting counted number of postings!", countPostings,
				this.navigator.getNumberOfAllMatches());

		this.typeTable.addType("new_Token_3");

		countPostings = 0;
		while (this.navigator.getNextMatch() != null) {
		...
		assertEquals("calculated number of postings must be same as actual!",
				countPostings, this.navigator.getNumberOfAllMatches());
	}
	...
	public void testSetRegionOfInterest() {
		...
		assertTrue("(2) Test if function return true if values in range",
				this.navigator.setRegionOfInterest(originX, originY, width,
						height));

		matchToCheck = this.navigator.getNextMatch();

		assertFalse("(3) posting to check must not be null",
				(matchToCheck == null));
		...
		while (matchToCheck != null) {
			...
			// assertTrue("(4) checkloop: if delivered postings in region of
			// interist",
			// ((postX >= originX) && (postX <= width) && (postY >= originY) &&
			// (postY <= height)));
			...
			// System.out.println("The test is checking posting: " +
			// matchToCheck.toString());
			matchToCheck = this.navigator.getNextMatch();
		}
	}
