...
/**
 * Instances of this class provide on the fly class loading mechanisms for classes storred 
 * in jarfiles in a given directory.
...
public class DirectoryJarClassLoader extends ClassLoader {
	 ...
	 * @param directory - The directory associated with the <code>DirectoryJarClassLoader</code>.
	 * @throws IllegalArgumentException if <code>directory</code> isn't a directory or doesn't exist.  
	 */
	public DirectoryJarClassLoader(File directory) {
		super();
		if(directory == null) throw new NullPointerException();
		if(! (directory.isDirectory() && directory.exists()) ) 
			throw new IllegalArgumentException("The assigned argument must be an existing directory");		
	 ...
	 * Creates a new <code>DirectoryJarClassLoader</code> object.
	 * @param directory - The directory associated with the <code>DirectoryJarClassLoader</code>. 
	 * @param loader - The parent <code>ClassLoader</code> of the the <code>DirectoryJarClassLoader</code>.
	 * @throws IllegalArgumentException if <code>directory</code> isn't a directory or doesn't exist.  
	 */
	 ...
	 * All jar-files of the associated directory are searched for the wanted class.
	 * The first hit is loaded. 
    		...
    		for(int i = 0; i < jarList.length; i++){
    			try {
					...
					if(entry != null){
						...
						jarFile.getInputStream(entry).read(data,0,data.length);
						return data;
					}
				}
				catch (IOException e) {
