...
public class JarFileClassLoader extends ClassLoader {
	...
	public JarFileClassLoader(File jarFile) {
		...
		try {
			...
			throw new IllegalArgumentException("The assigned argument must be an existing jar file");
		}
	}
	...
	public JarFileClassLoader(JarFile jarFile, ClassLoader loader) {
		super(loader);
		if(jarFile == null || loader == null) throw new NullPointerException();					
		this.jarFile = jarFile;
	}
		...
		if(entry != null){
			...
			try {
				...
				if(in != null){
					...
					catch (IOException e) {/*dann eben nicht*/}
				}
			}
			return data;
		}
