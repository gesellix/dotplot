...
public class FilterService extends DotplotService {
	...
	super(id);
	this.addHotSpot(new PluginHotSpot(id + FILTER_HOTSPOT_ID,
		ITokenFilter.class));
	...
	if (result == null) {
	    ...
	    ISourceList list = ((TokenStreamContext) this.getWorkingContext())
		    .getSourceList();
	...
	if (workingContext instanceof NullContext) {
	    this.getErrorHandler().fatal(this,
		    new IllegalContextException(new NullContext()));
	    return null;
	}

	final ITokenStream stream = ((TokenStreamContext) workingContext)
		.getTokenStream();
	...
	Task task = new Task("Filter Task", new ITaskResultMarshaler() {
		...
		return new FilteredTokenStream(stream,
			(ITokenFilter) taskResult.get("Part 1"));
	    }
	}, false);

	IFilterConfiguration config;
	try {
	    config = (IFilterConfiguration) this.frameworkContext
		    .getConfigurationRegistry().get(FILTER_CONFIGURATION_ID);
	    task.addPart(new FilterTaskPart("Part 1", this
		    .getRegisteredFilters(), config));
	...
	try {
	    ...
	    for (Extention e : this
		    .getHotSpot(this.getID() + FILTER_HOTSPOT_ID)
		    .getActiveExtentions()) {
		...
		if (name != null) {
			...
			if (uiClass != null) {
			    ...
			    try {
				Object o = e.getExtentionObject().getClass()
					.getClassLoader().loadClass(uiClass)
					.newInstance();
	...
	try {
	    registry.register(FILTER_CONFIGURATION_ID,
		    new DefaultFilterConfiguration());
