...
/**
 * A filter for keywords. Depending of the configuration, all Token or only
...
public class KeyWordFilter implements ITokenFilter {
	...
	/**
	 * Creates a KeyWordFilter. It starts in the default mode - All Token are
	 ...
	 * 
	 * @param modus
	 *            - the mode in which the KeyWordFilter runs
	 */
	 ...
	 *            - the TokenStream
	 * @param modus
	 *            - the mode in which the TokenFilter runs
	 */
	...
	public void applyParameter(Map<String, ? extends Object> parameter) {
		try {
			this
					.setModus(((Integer) parameter.get(PARAMETER_MODUS))
							.intValue());
		}
		catch (Exception e) {
			/* dann eben nicht */
	 ...
	 * Filters the Token.
	 * <p />
	 * Depending of the configuration, the keywords wil be allowed to pass or
	 * will be filtered The Token which will pass at any time are:
	 * <code>EOS, EOF, EOL, Line</code>.
	 * 
	 * @param token
	 *            - the<code>Token</code> which should be filtered
	 * 
	 ...
	 * 
	 * @param modus
	 *            - the mode, which should be configured
	 * 
	 * @see #getModus()
	 */
