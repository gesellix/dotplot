...
public class FilterTaskPart extends AbstractTaskPart {
	 ...
	 * @param id
	 */
	public FilterTaskPart(String id,  Map<String, ITokenFilter> filters, IFilterConfiguration config) {
		super(id);
		if( filters == null || config == null) throw new NullPointerException();
	...
	public void run() {
		...
		for(String filterID :this.config.getFilterList()){
			...
			if(filter == null){
				this.getErrorhandler().fatal(this, new UnknownIDException(filterID));
				this.container = null;
