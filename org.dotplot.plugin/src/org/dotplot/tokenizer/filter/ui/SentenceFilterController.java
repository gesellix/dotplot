...
class SentenceFilterController extends FilterViewController {
	 ...
	 * @param ui
	 */
	public SentenceFilterController(
			ConfigurationView cv, SentenceFilterUI ui) {
		super(cv, ui);
			...
			try {
				...
				if(ui.isSentenceFilter()){
					if(! config.getFilterList().contains(ui.getFilterID())){
						...
						if(mark != null){
							parameter.put(SentenceFilter.PARAM_TOKEN_VALUE, mark.getName());
							parameter.put(SentenceFilter.PARAM_TOKEN_TYPE, new Integer(mark.getType()));
						}
