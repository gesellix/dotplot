...
public class SelectTokenizerView extends ConfigurationView {
	...
	/**
	    ...
	    */
	   private static Logger logger = Logger.getLogger(SelectTokenizerView.class.getName());
	...
	private void updateTokenizerSelecter(){
		...
		for(String id : tokenizers.keySet()){
			...
			if(tokenizerType.isAssignableFrom(sourceClass)){
				...
				while(data != null){
					n++; 
					name = String.format("%s (%d)", new Object[]{tokenizer.getName(), new Integer(n)});
					data = this.tokenizerSelecter.getData(name);
				}
	...
	public void refresh(){IConverterConfiguration converterConfig = null;			
		...
		this.conversionDirectoryText.setText(converterConfig.getConvertedFilesDirectory().getAbsolutePath());	
		this.keepConvertedFilesButton.setSelection(converterConfig.keepConvertedFiles());

		//Tokenizer settings
		//nach dem converter refresh, damit auf converter einstellungen reagiert werden kann.	
