 ...
 * <ul>
 * <li>newSourceType</li>
 * <code>{@link org.dotplot.core.ISourceType}</code>
 * <p>
 * <b>Parameter:</b><br>
 * <u>name</u> - Used to identify the new type in the typeregistry.<br>
 * <u>suffix</u> [optional] - Used to assign the type to a file suffix. This
 * parameter only works with a <code>{@link org.dotplot.core.FileType}</code>
 * and is ignored otherwise.<br>
 * </p>
 * <li>newConverter</li>
 * <code>{@link org.dotplot.tokenizer.converter.IConverter}</code>
 * </ul>
 * 
...
public class ConverterService extends DotplotService {
	...
	super(id);
	this.addHotSpot(new PluginHotSpot(id + CONVERTER_HOTSPOT_ID,
		IConverter.class));
	this.addHotSpot(new PluginHotSpot(id + TYPE_HOTSPOT_ID,
		ISourceType.class));
	...
	try {
	    IConverterConfiguration config = (IConverterConfiguration) this.frameworkContext
		    .getConfigurationRegistry().get(CONVERTER_CONFIGURATION_ID);
	    ISourceType targetType = config.getTargetType();
	    SourceListContext context = (SourceListContext) this
		    .getWorkingContext();

	    final Vector<IPlotSource> v = new Vector<IPlotSource>(context
		    .getSourceList().size());
		...
		public Object marshalResult(
			...
			if (!result.isEmpty()) {
			    v.set(Integer.parseInt(key), (IPlotSource) result
				    .get(0));
			}
		    }
		    return v;
		...
		if (targetType.getClass().isAssignableFrom(
		    ...
		    // ein neues converterobjekt erstellen um für die
		    // parallel-verarbeitung gerüstet zu sein.
		    converter = converter.getClass().newInstance();

		    // letztes überprüfen der convertertypen
		    if (sourceType.getClass().isAssignableFrom(
			    converter.getSourceType().getClass())
			    && targetType.getClass().isAssignableFrom(
				    converter.getTargetType().getClass())) {
			// taskpart erstellen
			ressources = new Vector<IPlotSource>();
			ressources.add(s);
			part = new ConverterTaskPart(String.valueOf(i),
				converter, config.getConvertedFilesDirectory(),
	...
	ITypeBindingRegistry bindings = this.frameworkContext
	...
	try {
	    ...
	    this.converters.put(CONVERTER_PDF_TO_TEXT_ID,
	...
	try {
	    for (Extention e : this.getHotSpot(
		    this.getID() + CONVERTER_HOTSPOT_ID).getAllExtentions()) {
		try {
		    id = e.getParameter("id");
		    if (id == null) {
			throw new NullPointerException("id == null");
		    } else {
			if (this.converters.containsKey(id)) {
			    throw new DuplicateRegistrationException(id);
			} else {
			    this.converters.put(id, (IConverter) e
				    .getExtentionObject());
			}
		    }
		} catch (Exception e1) {
	    ...
	    for (Extention e : this.getHotSpot(this.getID() + TYPE_HOTSPOT_ID)
		    .getAllExtentions()) {
		...
		if (name == null) {
		    ...
		    try {
			types.register(name, (ISourceType) e
				.getExtentionObject());
	...
	try {
	    registry.register(CONVERTER_CONFIGURATION_ID,
		    new DefaultConverterConfiguration());
	} catch (DuplicateRegistrationException e) {
