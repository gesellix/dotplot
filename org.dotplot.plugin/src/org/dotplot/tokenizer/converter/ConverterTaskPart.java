...
public class ConverterTaskPart extends AbstractTaskPart {
	 ...
	 * @param converter
	 */
	public ConverterTaskPart(String id, IConverter converter, File targetDirectory, boolean overwrite) {
		super(id);
		if(converter == null|| targetDirectory == null) throw new NullPointerException();
	...
	public void setLocalRessources(Collection<? extends IRessource> ressouceList)
		...
		if(ressouceList == null) throw new InsufficientRessourcesException();
	...
	public void run() {
		...
		for(IRessource r : this.ressources){
			if(r instanceof IPlotSource){
				try {
					this.result.add(this.converter.convert((IPlotSource)r, this.targetDirectory));
				}
				catch (IOException e) {			
