...
/**
 * With the ConfigGridView the user can configure the grid settings.
 * It seperates the configuration in Server Settings and Client Settings
 */
public class ConfigGridView extends ConfigurationView
{
   ...
   private final static Logger logger = Logger.getLogger(ConfigGridView.class.getName());
    ...
    */
   public ConfigGridView(IGUIDotplotter dotplotter)
   {
      super(dotplotter);
   ...
   private void initLayoutServerGroup(Composite _parent)
   {
      ...
      cboAvailableAddresses.setItems((String[]) getAddresses().toArray(new String[0]));
   ...
   private void initLayoutClientGroup(Composite _parent)
   {
      ...
      btnUseServerSettings.setToolTipText("Fill in \"Server settings\" from above");
...
//      gd.horizontalSpan = 2;
      ...
      chkEnableNotify.setToolTipText("Enable email notification on completion of plots");
      ...
      chkNotifyAttachImage.setToolTipText("Attach image(s). They won't be combined!");
      ...
      txtNotifySMTPHost.setToolTipText("SMTP host used to send the notification mail");
      ...
      txtNotifySMTPUser.setToolTipText("Login (user) to connect to the SMTP host");
      ...
      txtNotifySMTPPass.setToolTipText("Login (password) to connect to the SMTP host");
      ...
      txtNotifyEmailTo.setToolTipText("Use \"" + recipientSeperator + "\" to seperate recipients");
   ...
   private void initListenersNotifyGroup()
   {
      ...
      btnNotifyTest.addSelectionListener(new SelectionAdapter()
      {
         public void widgetSelected(SelectionEvent e)
         {
            ...
            try
            {
               MailUtil.postMailBySMTP(txtNotifySMTPHost.getText(), txtNotifySMTPUser.getText(),
                     txtNotifySMTPPass.getText(), txtNotifyEmailFrom.getText(),
                     getNotifyEmailTo(), txtNotifyEmailSubject.getText(),
                     "Mail notification test");

               MessageDialog.openInformation(Display.getCurrent().getActiveShell(), "Test passed",
                     "Mail successfully sent. Please check your mailbox.");
            }
            catch (MessagingException msgExc)
            {
               MessageDialog.openError(Display.getCurrent().getActiveShell(), "Test failed!", msgExc.getMessage());
   ...
   private void updateGridServerUI()
   {
      if (gridServer != null && gridServer.isRunning())
      {
         ...
         btnStartStopGridServer.setToolTipText("Stops the mediator and disconnects all clients");

         lblGridServerState.setText("Mediator bound to  "
               + gridServer.getSocket().getInetAddress() + ":" + gridServer.getSocket().getLocalPort());
         lblGridServerState.setToolTipText("Shows the current address and port, the mediator is bound to."
               + "\nAn IP of \"0.0.0.0\" lets the mediator listen to any IP.");
      }
      else
      {
         btnStartStopGridServer.setText("Start mediator");
         btnStartStopGridServer.setToolTipText(
               "Starts the mediator to allow clients\nmake connections to this computer");
   ...
   private void updateGridClientUI()
   {
      if (gridClient != null && gridClient.getConnectionInfo() != null)
      {
         ...
         lblGridClientState.setText("Collaborator bound to  " + gridClient.getConnectionInfo());
         lblGridClientState.setToolTipText("Shows the current connection info.");
      ...
      else
      {
         ...
         btnStartStopGridClient.setToolTipText(
               "Connects this computer as collaborator to the mediator at the given address");
   ...
   {
      GridConfiguration config = ((GridConfiguration) GlobalConfiguration.getInstance().get(
...
//         gridServer.stop(localAddress);
   ...
   private void onStartStopGridClient()
   {
      ...
      else
      {
         ...
         logger.debug("connecting to " + getMediatorAddress() + ":" + getMediatorPort());
         try
         {
            ...
            gridClient = new GridClient(clientName, getMediatorAddress(), getMediatorPort());
         }
...
//      }
   ...
   private void showCurrentConfig()
   {
      ...
      if (isEnableNotification())
      {
         ...
         notifyEmailTo = glueSplittedStrings(config.getNotifyEmailTo(), recipientSeperator);
