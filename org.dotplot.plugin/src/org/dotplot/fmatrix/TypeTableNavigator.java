...
/**
 * an object of this class allows to navigate through tokentypes and all
...
public class TypeTableNavigator implements ITypeTableNavigator {
    ...
    private boolean considerThreshold; // flag wether to use consider
    // threshold.
    ...
    private int currentIndex; // holds the index of the current match (of the
    // number of matches of current tokenType).
     ...
     * 
     * @param typeTable
     *            - a reference on a TypeTable object.
     */
    ...
    /**
     * checks if the currentIndex points to a match within the set region of
     * interest. If it is not, the function searches for the next match that's
     * in this region.
     * 
     * @return if the match is in, or a new match within the region of interest
     *         could be found, function returns true
     */
	...
	if (currentIndex == 0
		|| currentIndex == currentTokenType.getNumberOfMatches()) {
	...
	while (typeTableIterator.hasNext()) {
	    currentTokenType = (TokenType) (((Map.Entry) typeTableIterator
		    .next()).getValue());
     ...
     * @param width
     *            the endpoint on the X dimension
     * @param height
     *            the endpoint on the Y dimension
     * 
     * @return true if the commited coordinates are in range
     */
    public synchronized boolean setRegionOfInterest(int x, int y, int width,
	    int height) {
    ...
    /**
     * sets the threshold for the frequency. (when set, only tokentypes below
     * this frequency will be considered.)
     * 
     * @param frequency
     *            - the frequency threshold. (set to 0 for not considering the
     *            threshold.)
     */
