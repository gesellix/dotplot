...
/**
 * Creates a new <code>Dotplot</code> based on the current <code>TypeTabele</code>.
 * <code>
...
public class ImagerJob extends AbstractJob<DotplotContext> {
	...
	public boolean process(DotplotContext context) {
		...
		try {
			IContext workingContext = new FMatrixContext(context.getCurrentTypeTable().createNavigator());
			...
			for(int i = 0; i < services.length; i++){
				...
				if(workingContext instanceof FMatrixContext){
					context.setCurrentTypeTable(((FMatrixContext)workingContext).getTypeTableNavigator().getTypeTable());
				}
			}
			
			context.setCurrentDotplot(((QImageContext)workingContext).getDotplot());
		}
		catch (UnknownIDException e) {
