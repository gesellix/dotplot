 ...
 * 	<ol>
 * 		<li><code>PluginLoadingService</code> 
 * 			- for loading <code>Plugins</code></li>
 * 		<li><code>PluginIntegrationService</code> 
 * 			- for integrating the <code>Service</code>s of the loaded 
 * 			<code>Plugins</code> into to system</li>
 * 		<li><code>InitializerService</code> 
 * 			- for initializing the integrated <code>Service</code>s</li>
 * 	</ol>
 * </p>
 * <p>
 * The <code>CoreSystem</code> provides for tree <code>Job</code>s:
 * 	<ol>
 * 		<li><code>PluginLoadingJob</code> 
 * 			- loads the <code>Plugins</code></li>
 * 		<li><code>StartUpJob</code> 
 * 			- executes <code>Job</code>s assigned to the 
 * 			<code>InizializerService</code> for startup.
 * 		<li><code>ShutDownJob</code> 
 * 			- executes <code>Job</code>s assigned to the 
...
public class CoreSystem extends Plugin {
	...
	private void init(){		
		try {
			//create services
			PlugableService<?> loader = new PluginLoadingService(SERVICE_LOADER_ID); 
			PlugableService<?> integrator = new PluginIntegrationService(SERVICE_INTEGRATOR_ID);
			PlugableService<?> initializer = new InitializerService(SERVICE_INITIALIZER_ID);
			
			//register services
			this.getServiceRegistry().register(SERVICE_LOADER_ID, loader);
			this.getServiceRegistry().register(SERVICE_INTEGRATOR_ID, integrator );
			this.getServiceRegistry().register(SERVICE_INITIALIZER_ID, initializer);
			
			//register jobs
			this.getJobRegistry().register(JOB_PLUGIN_LOADER_ID, new PluginLoadingJob(SERVICE_LOADER_ID, SERVICE_INTEGRATOR_ID, SERVICE_INITIALIZER_ID));
