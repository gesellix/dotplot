...
public class Version implements Comparable<Version> {
	 ...
	 * 
	 */
	public static final VersionPattern DEFAULT_PATTERN = new VersionPattern(
			VERSION_PATTERN_MAIN_IDENTIFIER, VERSION_PATTERN_ALPHA_IDENTIFIER,
	 ...
	 * @throws MalformedVersionException
	 */
	public Version(String version, VersionPattern pattern)
			throws MalformedVersionException {
	...
	public int compareTo(Version version) {
		...
		int i = 0;
		i = this.normalize(this.mainVersionString).compareTo(
				this.normalize(main));
		if (i == 0) {
	...
	private void init(String version) {
		...
		this.releaseState = version.replaceFirst(
				this.pattern.getMainIdentifier(), "").trim();
		this.mainVersionString = this.completeVersionString.substring(
				0,
				this.completeVersionString.length()
						- this.releaseState.length()).trim();
		this.releaseNumber = this.releaseState.replaceFirst(
				this.pattern.getReleaseStateIdentifier(), "").trim();
		if ("".equals(this.releaseNumber)) {
		...
		else if (this.releaseState.split(this.pattern
				.getReleaseCandidateIdentifier()).length != 1) {
	 ...
	 * 
	 * @return <code>true</code> if it's an alphaversion, <code>false</code>
	 *         otherwise.
	 */
	 ...
	 * 
	 * @return <code>true</code> if it's a betaversion, <code>false</code>
	 *         otherwise.
	 */
	 ...
	 * 
	 * @return <code>true</code> if it's a release candidate, <code>false</code>
	 *         otherwise.
	 */
