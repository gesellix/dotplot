...
/**
 * This <code>TaskPart</code> integrates <code>Plugin</code>s into a <code>PluginContext</code>.
 * <p>
...
public class PluginIntegrationTaskPart extends AbstractTaskPart {
	 ...
	 * @param id The id of the <code>TaskPart</code>.
	 * @param plugins The <code>Plugin</code>s to integrate.
	 * @param context The context to integrate the plugins in.
	 */
	public PluginIntegrationTaskPart(String id, Collection<? extends IPlugin> plugins, IPluginContext<? extends IPlugin> context) {
		super(id);
		if(plugins == null || context == null) throw new NullPointerException();
		...
		for(IPlugin plugin : this.plugins){
			for(String service : plugin.getExtentions().keySet()){
				for(String spot: plugin.getExtentions().get(service).keySet()){
					for(Extention e : plugin.getExtentions().get(service).get(spot)){
						try {
							this.context.getServiceRegistry().get(service).addExtention(spot, e);
						}
