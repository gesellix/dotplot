...
public class PlotterJob extends AbstractJob<DotplotContext> {
	...
	/* (non-Javadoc)
	 * @see org.dotplot.core.services.IJob#process(org.dotplot.core.services.IFrameworkContext)
	 */
	public boolean process(DotplotContext context) {
		...
		try {			
			IContext workingContext = new SourceListContext(context.getSourceList());
			...
			for(int i = 0; i < services.length; i++){
				...
				if(workingContext instanceof FMatrixContext){
					ITypeTable table = ((FMatrixContext)workingContext).getTypeTableNavigator().getTypeTable();
