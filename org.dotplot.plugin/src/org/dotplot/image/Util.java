...
public final class Util {
	 ...
	 */
	public static final int COLOR_DEPTH = NUM_BANDS
			* NUM_BITS_PER_BAND_PER_PIXEL;

	/**
	 * Number of colors ("color depth").
	 */
	public static final int COLOR_COUNT_PER_BAND = (int) Math.pow(2,
			NUM_BITS_PER_BAND_PER_PIXEL);
	...
	static ImageData createEmptyImageData(Dimension size) {
		return new ImageData(size.width, size.height, COLOR_DEPTH,
	...
	static TiledImage createEmptyTiledImage(Dimension size) {
		...
		return new TiledImage(0, 0, size.width, size.height, 0, 0, bim
				.getSampleModel(), bim.getColorModel());
	}

	synchronized static void createImage(ITypeTableNavigator navData,
			int[][] lut, ImageCallback image,
		...
		image.updateProgress(IDotplot.STEPS_WIDTH,
				IDotplot.STEP_READING_MATCHES, null);
		...
		// if background color not "black", paint background color (lut[i][0])
		// on empty image
		...
		if (!(lut[0][0] == 0 && lut[1][0] == 0 && lut[2][0] == 0)) {
			final int color = new Color(lut[0][0], lut[1][0], lut[2][0])
					.getRGB();
		...
		while ((match = navData.getNextMatch()) != null) {
			...
			col = convertToColor(match.getWeight(), MAX_COL_VAL, imageConfig
					.useLUT());

			// put current pixel into image
			image.setPixel(match.getX(), match.getY(), new Color(lut[0][col],
					lut[1][col], lut[2][col]).getRGB());
		...
		final boolean showFileSeparators = imageConfig.showFileSeparators()
	 ...
	 *            the target
	 */
	public static void exportDotplotInROI(final ITypeTableNavigator navigator,
			final Rectangle roi, final String filename,
			final IQImageConfiguration imageConfig) {
		...
		// JAITools.saveJAI(JAITools.getWithLUT(image.createSnapshot(), lut),
		// filename, JAITools.EXPORTFORMAT_JPEG);
	...
	public static void exportDotplotInROIByInfoMural(
			final ITypeTableNavigator navigator, final Dimension targetSize,
			final Rectangle roi, final String filename,
		...
		final int[] fileIndices = navigator.getTokenInformation()
				.getAllStartIndices();
	 ...
	 * @param scale
	 *            the scale factor of the original data to the currently
	 *            displayed image
	 * @param nav
	 *            the source data
	 * 
	 ...
	 */
	public static IROIResult getDetailsForLoc(Point location,
			Dimension imageSize, float scale, ITypeTableNavigator nav) {
		...
		// int xIndex = Math.min(Math.max((int) (location.x / scale), 0),
		// imageSize.width - 1);
		// int yIndex = Math.min(Math.max((int) (location.y / scale), 0),
		// imageSize.height - 1);
		...
		// logger.debug("Location ix: " + location.x + "-->" + xIndex + ", " +
		// location.y + "-->" + yIndex);
		...
		// logger.debug(result);
		final ROIResult roiResult = new ROIResult(new File(tokenInfo
				.getFileName(xFileID)),
				new File(tokenInfo.getFileName(yFileID)), xLineIndex,
				yLineIndex);
	...
	static void getDotplotForScreenSWT(Raster raster, ImageCallback image) {
		image.updateProgress(IDotplot.STEPS_WIDTH / 2,
				IDotplot.STEP_CONVERT_DATA, "creating screen data");
	...
	private static ImageCallback getImageCallback(final TiledImage image,
			final Rectangle roi) {
		return new ImageCallback() {
			...
			public void invertLine(int index) {
				...
				if (roi.contains(index, index)
						&& image.getBounds().contains(absX, absY)) {
	...
	private static void getSWTImage(ImageCallback image, Raster imageData) {
		/*
		 * TODO is it faster to save to a temp file and then load the image of
		 * this file to show it on screen?
		 * 
		 * File tmpDP = new File("./tmp_dp.jpeg"); JAITools.saveJAI(
		 * displayImage, tmpDP.getAbsolutePath(), JAITools.EXPORT_FORMATS[1]);
	...
	static void invertLines(int[] fileIndices, ImageCallback image, double scale) {
		...
		int lastIx = -1;
		for (int i = 0; i < fileIndices.length; i++) {
			if (Math.abs(fileIndices[i] - lastIx) > 5
					&& (width - (fileIndices[i] * scale)) > 2) {
	...
	private static int printProgress(int percent, long count, long step,
			ImageCallback image) {
		if ((step != 0) && ((count % step) == 0) && (percent < 100)) {
			percent++;
			logger.debug("todo: " + count + " (" + percent + "% ready)");

			image.updateProgress((percent / 100) * IDotplot.STEPS_WIDTH,
					IDotplot.STEP_READING_MATCHES, null);
		}
		return percent;
	}
