...
/**
 * This class contains several static methods that create on-the-fly
 * look-up tables.
 *
...
public class LUTs
{
    ...
    */
   public static final String[] availableLUTs = {
      "matrix4dotplot",
      "red", "green", "blue", "cyan", "magenta", "yellow",
      "gray", "inverted_gray",
      "two_levels", "four_levels", "eight_levels", "sixteen_levels", "thirty_two_levels",
      "sixty_four_levels", "hundred_twenty_eight_levels",
      "red_cyan", "green_magenta", "blue_yellow",
      "sin_rgb", "sin_rbg", "sin_grb", "sin_gbr", "sin_brg", "sin_bgr",
      "sin_rgb_0", "sin_rbg_0", "sin_grb_0", "sin_gbr_0", "sin_brg_0", "sin_bgr_0",
      "sqrt_rgb", "sqrt_rbg", "sqrt_grb", "sqrt_gbr", "sqrt_brg", "sqrt_bgr",
      "sqrt_rgb_0", "sqrt_rbg_0", "sqrt_brg_0", "sqrt_bgr_0", "sqrt_gbr_0", "sqrt_grb_0",
      "hue_rgb", "hue_rbg", "hue_grb", "hue_gbr", "hue_brg", "hue_bgr",
      "hue_rgb_0", "hue_rbg_0", "hue_grb_0", "hue_gbr_0", "hue_brg_0", "hue_bgr_0",
      "red_saw_2", "red_saw_4", "red_saw_8",
      "green_saw_2", "green_saw_4", "green_saw_8",
      "blue_saw_2", "blue_saw_4", "blue_saw_8",
      "red_green_saw_2", "red_green_saw_4", "red_green_saw_8",
      "red_blue_saw_2", "red_blue_saw_4", "red_blue_saw_8",
      "green_blue_saw_2", "green_blue_saw_4", "green_blue_saw_8",
      "random_256", "random_32", "random_8",
   };
...
*/
   ...
   /**
    * The gray lut with selectable range (0.00 - 1.00).
    * The resulting lut will start the grayscale (with "black") at the given percentage and the complete spectrum
    * to "white" will be used.
    *
    * @param percent The range (in percent) from which the grayscale should be built.
    *                Values have to be from 0.0 up to 1.0 (where "1.0" doesn't make much sense...).
    *                "0.0" will produce the "complete" grayscale.
    *
    * @return a lut
    */
   ...
   public final static int[][] two_levels()
   {
      ...
      for (int i = 0; i < LUTSIZE; i++)
      {
         lut[0][i] = (int) (128 * (int) (i / 128));
         lut[1][i] = (int) (128 * (int) (i / 128));
         lut[2][i] = (int) (128 * (int) (i / 128));
      }
      return lut;
   }
   ...
   public final static int[][] four_levels()
   {
      ...
      for (int i = 0; i < LUTSIZE; i++)
      {
         lut[0][i] = (int) (64 * (int) (i / 64));
         lut[1][i] = (int) (64 * (int) (i / 64));
         lut[2][i] = (int) (64 * (int) (i / 64));
      }
      return lut;
   }
   ...
   public final static int[][] eight_levels()
   {
      ...
      for (int i = 0; i < LUTSIZE; i++)
      {
         lut[0][i] = (int) (32 * (int) (i / 32));
         lut[1][i] = (int) (32 * (int) (i / 32));
         lut[2][i] = (int) (32 * (int) (i / 32));
      }
      return lut;
   }
   ...
   public final static int[][] sin_rgb()
   {
      ...
      for (int i = 0; i < LUTSIZE; i++)
      {
         lut[0][i] = (int) (127 * (1 + Math.sin(Math.PI * (i - 127) / (LUTSIZE - 1))));
         lut[1][i] = (int) (127 * (1 + Math.sin(Math.PI * (i) / (LUTSIZE - 1))));
         lut[2][i] = (int) (127 * (1 + Math.sin(Math.PI * (i + 127) / (LUTSIZE - 1))));
      }
      return lut;
   }
   ...
   public final static int[][] sin_rbg()
   {
      ...
      {
         lut[0][i] = (int) (127 * (1 + Math.sin(Math.PI * (i - 127) / (LUTSIZE - 1))));
         lut[2][i] = (int) (127 * (1 + Math.sin(Math.PI * (i) / (LUTSIZE - 1))));
         lut[1][i] = (int) (127 * (1 + Math.sin(Math.PI * (i + 127) / (LUTSIZE - 1))));
      }
      return lut;
   }
   ...
   public final static int[][] sin_grb()
   {
      ...
      {
         lut[1][i] = (int) (127 * (1 + Math.sin(Math.PI * (i - 127) / (LUTSIZE - 1))));
         lut[0][i] = (int) (127 * (1 + Math.sin(Math.PI * (i) / (LUTSIZE - 1))));
         lut[2][i] = (int) (127 * (1 + Math.sin(Math.PI * (i + 127) / (LUTSIZE - 1))));
      }
      return lut;
   }
   ...
   public final static int[][] sqrt_rgb()
   {
      ...
      for (int i = 0; i < LUTSIZE; i++)
      {
         ...
         lut[1][i] = i;
         lut[2][i] = (int) ((LUTSIZE - 1) - 15 * Math.sqrt((LUTSIZE - 1) - i));
      }
      return lut;
   }
   ...
   public final static int[][] sqrt_rbg()
   {
      ...
      for (int i = 0; i < LUTSIZE; i++)
      {
         lut[0][i] = (int) ((LUTSIZE - 1) - 15 * Math.sqrt(i));
         lut[2][i] = i;
         lut[1][i] = (int) ((LUTSIZE - 1) - 15 * Math.sqrt((LUTSIZE - 1) - i));
      }
      return lut;
   }
   ...
   public final static int[][] sqrt_gbr()
   {
      ...
      for (int i = 0; i < LUTSIZE; i++)
      {
         lut[1][i] = (int) ((LUTSIZE - 1) - 15 * Math.sqrt(i));
         lut[2][i] = i;
         lut[0][i] = (int) ((LUTSIZE - 1) - 15 * Math.sqrt((LUTSIZE - 1) - i));
      }
      return lut;
   }
