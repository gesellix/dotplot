...
public interface IQImageConfiguration extends IConfiguration {
	...
	/**
	 * if scaleMode is set to 1, this setting tells whether to scale up small
	 * images or to only scale down.
	 * 
	...
	/**
	 * the user can set a background to change the current lut. this method will
	 * return the current setting in lut[x][0].
	 * 
	...
	/**
	 * the user can set a foreground to change the current lut. this method will
	 * return the current setting in lut[x][lut[x].length-1].
	 * 
	...
	/**
	 * returns the current setting for the scale mode. 0 means "no scaling", 1
	 * means "scaling active".
	 * 
	...
	/**
	 * If a plot is done from command line ("headless"), this will return true.
	 * For this case, e.g. no progress bar will be displayed.
	 * 
	 ...
	 * 
	 * @param i
	 *            one of the options supported by the JAITools class
	 * 
	...
	/**
	 * activates the handling for a single export, without interaction on the
	 * GUI.
	 * 
	 * @param onlyExport
	 *            only do an export?
