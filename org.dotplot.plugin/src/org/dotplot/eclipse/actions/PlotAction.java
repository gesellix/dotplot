...
//            .getDotplotCreator();
   ...
   private void handleEmptyFileList(DotplotContext context)
         throws Exception {
      if (MessageDialog.openQuestion(window.getShell(), "No files selected",
         ...
         String filename = FileUtil.showFileDialog(window.getShell(),
               "Select a PlotJob or press \"Cancel\"", new String[]{"*.dgz", "*"});

         if (filename != null) {
            File file = new File(filename);
            logger.info("using Plot from file " + file.getAbsolutePath());
            context.setCurrentTypeTable(FileUtil.importPlotJob(file).getTypeTable());
         }
         else {
   ...
   protected void createAndShowPlot(DotPlotter view) {
      ...
      try {
         tempConfig = (IQImageConfiguration) context.getConfigurationRegistry()
      ...
      if (dplot == null) {
         ...
         try {
            config = (GridConfiguration) context.getConfigurationRegistry().get(QImageService.ID_CONFIGURATION_GRID);
         }
         catch (UnknownIDException e) {
         ...
         if (gridConfig.isGridActive()) {
            ...
            PlotJob plotJob = new PlotJob(imageConfig, context.getCurrentTypeTable());
         ...
         if (isGridWorking) {
            ...
            if (imageConfig.getScaleMode() == 0) {
               scale = dplot.setTargetSize(context.getCurrentTypeTable().createNavigator().getSize());
            }
            else {
            ...
            try {
               filterconfig = (IFilterConfiguration) context.getConfigurationRegistry()
            ...
            if (filterconfig.getFilterList().size() > 0) {
               ...
               view.setMouseMoveListener(new DotPlotMouseMoveListener(window, data, view, scale));
            }
            view.showImage(data);
         }
      }
   ...
   public void imagesReceived(final Map gridImages, final Dimension size) {
      ...
      try {
         context.getConfigurationRegistry().get(QImageService.ID_CONFIGURATION_GRID);
      }
      catch (UnknownIDException e) {
         config = new GridConfiguration();
      }
      ...
      if (gridConfig.isEnableNotification()) {
         ...
         try {
            if (gridConfig.isNotifyAttachImage()) {
               ...
               for (Object o : gridImages.keySet()) {
                  indexVector.add(o);
               }
   ...
   private void aggregateAndShowPlot(Map gridImages, Dimension size) {
      ...
      final float scale = (float) size.width
            / (float) ContextFactory.getContext().getCurrentTypeTable().createNavigator().getSize().width;

      plotter.showImage(imgData);
      plotter.setMouseMoveListener(new DotPlotMouseMoveListener(window, imgData, plotter, scale));
   }
   ...
   private void showMessage(String text) {
      ...
      try {
         ...
         //dieser aufruf kann eigentlich raus, aber bleibt zur sicherheit nochmal drin
