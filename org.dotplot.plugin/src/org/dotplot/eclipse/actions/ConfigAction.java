...
/**
 * <code>ConfigAction</code> is there to provide configured plotting using
 * plot action.
 * 
...
public class ConfigAction implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction iaction) {
		try {
			window.getWorkbench().showPerspective(EclipseConstants.ID_PERSPECTIVE_DOTPLOT,window);
		}
		...
		try {
			views = ((EclipseUIService)context.getServiceRegistry().get("org.dotplot.standard.EclipseUI")).getConfigurationViews();
		}
...
//				GlobalConfiguration.KEY_CONFIGURATION_VIEWS);
		...
		ConfigurationDialog config = new ConfigurationDialog(registryCopy, views);
