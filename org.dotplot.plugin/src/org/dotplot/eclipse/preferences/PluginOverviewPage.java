 ...
 */
public class PluginOverviewPage extends PreferencePage implements IWorkbenchPreferencePage{
	 ...
	 */
	public PluginOverviewPage(String title) {
		super(title);
		this.init();
	}
	 ...
	 */
	public PluginOverviewPage(String title, ImageDescriptor image) {
		super(title, image);
		this.init();
	}
	...
	/**
	 * Updates the preferences based of the current plugin settings in the <code>DotplotContext</code>. 
	 */
	public void updatePreferences(){
		PluginContext<Plugin> context = ContextFactory.getContext();
		for(Plugin plugin : context.getPluginRegistry().getAll().values()){
			this.getPreferenceStore().setValue(plugin.getID(), plugin.isActivated());
		}
	}
