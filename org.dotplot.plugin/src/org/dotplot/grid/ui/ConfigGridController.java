...
public class ConfigGridController extends ViewController {
	...
	if (o instanceof ConfigGridView) {
	    ...
	    gridView.getRegistry().unregister(
		    QImageService.ID_GRID_CONFIGURATION);

	    final GridConfiguration gridConfig = new GridConfiguration(gridView
		    .isEnableGrid(), gridView.getLocalPort(), gridView
		    .getMediatorAddress(), gridView.getMediatorPort());
	    ...
	    try {
		gridView.getRegistry().register(
			QImageService.ID_GRID_CONFIGURATION, gridConfig);
