 ...
 * <p />
 * Class: MessageMediator Example: 9-5 Description: A mediator that uses
 * message-passing for its communication.
 * <p />
...
public class MessageMediator extends Mediator implements Runnable {
	...
	private final static Logger logger = Logger.getLogger(MessageMediator.class
			.getName());
	...
	public void run() {
		...
		while (!stop) {
			try {
				...
				Message idMsg = new Message(AbstractMessage.MSG_IDENTITY);
				...
				logger.error(
						"mm: an error occurred while waiting for clients:", e);
			}
		}
