 ...
 * <p/>
 * Class: MediatorMessage Example: 9-6 Description: Subclass of Message that
 * handles messages for our collaboration framework.
 * <p/>
 * Changed by Tobias Gesellchen
 */
class MediatorMessage extends Message {
	// protected GridNode gridNode = null;
	private final Logger logger = Logger.getLogger(MediatorMessage.class
			.getName());

	public MediatorMessage(GridNode m) {
		if (false == (m instanceof Mediator)) {
			throw new IllegalArgumentException("I need "
	...
	public boolean Do() {
		...
		try {
			...
			if (mtype.compareTo(AbstractMessage.MSG_TYPE_SEND) == 0) {
			...
			else if (mtype.compareTo(AbstractMessage.MSG_TYPE_BROADCAST) == 0) {
