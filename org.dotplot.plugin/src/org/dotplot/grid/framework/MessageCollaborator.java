...
{
   private final Logger logger = Logger.getLogger(MessageCollaborator.class.getName());
    ...
    * @param host the Mediator address
    * @param port the Mediator port
    *
    * @throws ConnectionException if an error occured
    */
   public MessageCollaborator(String name, String host, int port) throws ConnectionException
   {
      initHandler();
...
//         System.exit(1);
   ...
   public String setIdentity(Identity identity)
   {
      ...
      try
      {
         send(Message.MSG_IDENTITY_NAME, id, getMediatorIdentity());
   ...
   public boolean connect(Properties p)
   {
      ...
      if (host != null && itmp != null)
      {
         try
         {
            ...
            logger.debug("Got socket to Mediator, mediatorID = " + mediatorID);
         ...
         {
            logger.error("Error connecting to mediator or reading message", e);
            success = false;
         }
      }
      else
