...
public class GridServer extends MessageMediator {
    ...
    private static final Logger logger = Logger.getLogger(GridServer.class
	    .getName());
    ...
    @Override
    public synchronized boolean notify(String tag, Object data, Identity src)
	    throws IOException {
	...
	// let a new client connect to the mediator to let it read the above
	// request
	// to stop running (after having connected to our new client completely)
	try {
	    new MessageCollaborator("SHUTDOWN", getSocket().getInetAddress()
		    .getHostAddress(), getSocket().getLocalPort());
	} catch (ConnectionException e) {
	    logger
		    .error(
			    "Error shutting down server. Connection could not be established.",
			    e);
	}
    }
}
